...
def print_campaign_table(campaigns: list[CampaignResult]) -> None:
    """Print the adaptive-attacker effort-to-break table, grouped by adapter.
    ...
    """
    groups: dict[tuple[str, str], list[CampaignResult]] = defaultdict(list)
    for c in campaigns:
        groups[(c.adapter, c.attack)].append(c)

    print(f"{'adapter':<16} {'attack':<16} {'n':>4} {'broke':>6} {'rate':>6}  "
          f"{'95% CI':<14} {'attempts (med/mean)'}")
    print("-" * 78)

    for (adapter, attack), items in sorted(groups.items()):
        n = len(items)
        ...
        if attempts:
            ...
            att = "never broke"
        print(f"{adapter:<16} {attack:<16} {n:>4} {broke:>6} {rate:>5.0%}  "
              f"[{lo:.0%}, {hi:.0%}]      {att}")
